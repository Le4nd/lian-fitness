    ...
    <>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.textMuted, marginBottom: 6 }}>
          <span>97kg → 65kg</span><span style={{ color: C.blueDeep, fontWeight: 600 }}>{weightPct}%</span>
        </div>
        <div style={s.pBar}><div style={s.pFill(weightPct)} /></div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: C.textMuted, marginTop: 4 }}><span>97kg</span><span>65kg</span></div>
      </div>
      <div style={{ ...s.card, padding: 16, marginBottom: 16 }}>
        <span style={s.lbl}>Log weight for {isToday ? 'today' : selectedDate}</span>
        <div style={{ display: 'flex', gap: 8 }}>
          <input type="number" step="0.1" placeholder="e.g. 95.4" value={newWeight}
            onChange={e => setNewWeight(e.target.value)} onKeyDown={e => e.key === 'Enter' && logWeight()}
            style={{ ...s.inp, flex: 1 }} />
          <button onClick={logWeight} style={s.btn}>Log</button>
        </div>
      </div>
      <span style={s.lbl}>Full history ({weightLog.length} entries)</span>
      <div style={s.card}>
        ...
        {[...weightLog].reverse().map((e, i, arr) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: i < arr.length - 1 ? `1px solid ${C.neutral}` : 'none', background: e.date === selectedDate ? C.blueLight : 'transparent' }}>
              ...
              {diff ? <span style={{ fontSize: 12, fontWeight: 600, color: +diff < 0 ? C.green : C.red }}>{+diff < 0 ? diff : `+${diff}`}</span> : <span style={{ fontSize: 12, color: C.textMuted }}>—</span>}
            </div>
          )
        })}
      </div>
...
export function BodyTab({ measureLog, newMeasure, setNewMeasure, logMeasurements, selectedDate }) {
  const isToday = selectedDate === todayStr()
  const latestMeasure = measureLog[measureLog.length - 1] ?? {}
  const whrRatio = latestMeasure.waist && latestMeasure.hips ? (latestMeasure.waist / latestMeasure.hips).toFixed(2) : null
  return (
    <>
      {whrRatio && (
        <div style={{ background: `linear-gradient(135deg,${C.blueLight},${C.pinkLight})`, borderRadius: 18, padding: 18, marginBottom: 16, border: `1px solid ${C.blueMid}`, textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 13, marginTop: 6, fontWeight: 600, color: +whrRatio < 0.70 ? C.green : +whrRatio < 0.80 ? C.blueDeep : C.red }}>
            {+whrRatio < 0.70 ? 'Hourglass range ✓' : +whrRatio < 0.80 ? 'Getting there' : 'Above hourglass range'}
          </div>
        </div>
      )}
      {latestMeasure.waist && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
          {[['Waist', latestMeasure.waist, C.pinkDeep, C.pinkLight, C.pinkMid], ['Hips', latestMeasure.hips, C.blueDeep, C.blueLight, C.blueMid], ['Thighs', latestMeasure.thighs, C.green, C.greenLight, C.greenBorder]].map(([l, v, color, bg, border]) => (
      ...
      <span style={s.lbl}>Log for {isToday ? 'today' : selectedDate}</span>
      <div style={{ ...s.card, padding: 16 }}>
        {[['waist', 'Waist (cm)', '85'], ['hips', 'Hips (cm)', '100'], ['thighs', 'Thighs (cm)', '65']].map(([k, label, ph]) => (
          <div key={k} style={{ marginBottom: 10 }}>
            ...
            <input type="number" step="0.1" placeholder={`e.g. ${ph}`} value={newMeasure[k]}
      ...
      <span style={s.lbl}>History ({measureLog.length} entries)</span>
      <div style={s.card}>
        ...
        {[...measureLog].reverse().map(({ date, waist, hips, thighs }, i, arr) => (
          <div key={date} style={{ padding: '12px 16px', borderBottom: i < arr.length - 1 ? `1px solid ${C.neutral}` : 'none', background: date === selectedDate ? C.blueLight : 'transparent' }}>
    ...
    <>
      ...
      <div style={s.card}>
        {weekDates.map((d, i) => {
          ...
          return (
            <div key={d} onClick={() => setSelectedDate(d)} style={{ padding: '11px 16px', borderBottom: i < weekDates.length - 1 ? `1px solid ${C.neutral}` : 'none', background: isToday ? C.blueLight : 'transparent', cursor: 'pointer' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>
                    ...
                    {!steps && !cal && <span style={{ color: C.neutralBorder }}>tap to fill in</span>}
                  </div>
                </div>
    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
        {MILESTONES.map((m, i) => {
          ...
          return (
            <div key={i} style={{ background: reached ? C.goldLight : C.white, borderRadius: 14, border: `1px solid ${reached ? C.goldBorder : C.neutralBorder}`, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: reached ? '0 2px 8px rgba(184,134,11,0.12)' : 'none' }}>
              ...
              {reached ? <span style={{ fontSize: 11, fontWeight: 700, color: C.gold, background: '#fff', padding: '3px 10px', borderRadius: 20, border: `1px solid ${C.goldBorder}` }}>Reached ✓</span>
      ...
      <div style={{ background: C.white, borderRadius: 18, border: `1px solid ${C.neutralBorder}`, padding: 18 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: C.textMuted, marginBottom: 8 }}>
          <span>97kg → 65kg</span><span style={{ fontWeight: 700, color: C.blueDeep }}>{weightPct}%</span>
        </div>
        <div style={{ position: 'relative', height: 16, borderRadius: 8, background: C.neutralBorder, marginBottom: 14, overflow: 'visible' }}>
          ...
          {MILESTONES.map(m => {
            ...
            const reached = latestW <= m.weight
            return (
              <div key={m.weight} title={`${m.label} (${m.weight}kg)`} style={{ position: 'absolute', top: '50%', left: `${pos}%`, transform: 'translate(-50%,-50%)', width: 20, height: 20, borderRadius: '50%', background: reached ? C.gold : '#fff', border: `2px solid ${reached ? C.goldBorder : C.neutralBorder}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, zIndex: 1 }}>
                {reached ? '★' : '·'}
              </div>
            )
          })}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: C.textMuted }}><span>97kg</span><span>65kg</span></div>
      </div>
    </>
  )
}
